...
INDEX_DIR = os.path.abspath('expanded_indexes')
...
def run_ltr_with_sbert_expansion():
    ...
    tfidf = pt.terrier.Retriever(index, wmodel="TF_IDF", num_results=HITS_PER_QUERY,
                                 controls={'w.0': 2.0, 'w.1': 3.0, 'w.2': 1.25}, metadata=["docno", "body"])
    pl2 = pt.terrier.Retriever(index, wmodel="PL2", num_results=HITS_PER_QUERY,
                               controls={'w.0': 2.0, 'w.1': 3.0, 'w.2': 1.25}, metadata=["docno", "body"])

    dfr_bm25 = pt.terrier.Retriever(
        index, wmodel="DFR_BM25", num_results=HITS_PER_QUERY)
    ...
    dfiz = pt.terrier.Retriever(
        index, wmodel="DFIZ", num_results=HITS_PER_QUERY)
    ...
    train_q, val_q = train_test_split(
        train_queries, test_size=0.25, random_state=42)
